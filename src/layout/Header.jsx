...
export default function Header({ cartCount = 0, wishlistCount = 0 }) {
  ...
  return (
    <Box bg={{md:"", base: 'white'}} position="sticky" top="0" zIndex="50">
      <Flex position={'relative'} justifyContent={'space-between'} alignItems={'center'} px={4} py={3} align="center" maxW={{ base: '100%', xl: '90%', '2xl': '80%' }} mx="auto">
        ...
        <Box bgImage={`url('/img/img.jpg')`} bgBlendMode={'multiply'} bgRepeat="no-repeat" bgSize="cover" bgPos={{md:"left", base: 'center'}} px={3} py={1} rounded={'md'} as={RouterLink} to="/" fontSize="2xl" fontWeight="bold" color="white" className="bg">
          <Text>Jewelry Store</Text>
        </Box>

        <Box display={{ base: "none", md: "block" }} maxWidth={'100%'} mx={'auto'} py={3} px={5} roundedBottom={'xl'} bg={'white'} pos={'absolute'} bottom={'-10'} transform={'translateX(-50%)'} left={'50%'}>
        ...
        <Flex spaceX={4} color={'black'}>
           <AddToCartDrawer/>
            <Flex color={'pink.600'} cursor={'pointer'} justifyContent={'center'} alignItems={'center'} bg={'gray.200'} p={2} rounded={'full'}>
                <FaHeart />
            </Flex>
            <Flex color={'blue.600'} cursor={'pointer'} justifyContent={'center'} alignItems={'center'} bg={'gray.200'} p={2} rounded={'full'}>
                <FaUser />
            </Flex>
        </Flex>
      </Flex>
